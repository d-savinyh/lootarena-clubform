...
- **Фавикон лендинга** — `public/favicon.svg`: во вкладке браузера висела фиолетовая молния (`#863bff`) — иконка, не совпадающая с остальными продуктами. Заменена на фирменный знак LootArena из репозитория ЕРП (`#30D158`). Разметка не менялась — `index.html` уже ссылается на `/favicon.svg`.

### ⚙️ Инфраструктура

- **`toolkit.json` указывал на приложение ЕРП** — `project.name` был «ERP LootArena», `dokploy_app_id` — `3PXMjIVDYNJ59TlzEhI6Z` (конфиг скопирован из другого проекта и не переименован). Из-за этого n8n после деплоя лид-форм проверял статус **приложения ЕРП** и слал в Telegram статус не того сервиса. Исправлено на «Лид-формы LootArena» / `VICWfLBZT72JHLjTI_3R8`.
- **Черновик релиза в ленте «Релизы» ЕРП** — `.agent/workflows/deploy.md`: новый шаг 4.5 деплоя создаёт черновик в `support_releases` с `product: "clubform"` (лента общая для всех продуктов LootArena — без поля релиз лид-форм уедет в ленту ЕРП). Запись создаётся со `status: "draft"`, публикует человек в UI. Добавлены правила клиентской подачи: пункты пишутся от возможности, а не от бывшей поломки, без внутренних идентификаторов и метрик инцидента.

---

## [0.4.0] — 2026-08-16 `v0.4.0`

### 🆕 Новое

- **Антиспам-барьер на лид-форме** — `LeadForm`/`captcha.ts`/`api.ts`: три уровня защиты без урона конверсии. (1) **Honeypot** — скрытое поле `company`, уведённое за пределы экрана (`-left-[9999px]`, не `display:none` — часть ботов такие поля пропускает), закрытое от табуляции, автозаполнения и скринридеров; непустое значение = бот. (2) **Замер скорости заполнения** — `form_ms` от монтирования формы до отправки, сервер отбраковывает быстрее 1.5 с. (3) **Yandex SmartCaptcha `[NEW]`** — `src/utils/captcha.ts`, невидимая (`invisible`, `hideShield`), подключается **только по требованию сервера** (ответ `requireCaptcha` + `captcha_key`), после чего заявка переотправляется с `captcha_token`. Ключ приходит с сервера, а не из env — включение капчи это строка в `clubform_antispam_config`, деплой не нужен. Отказ/таймаут (60 с) не теряет заявку молча: `solveCaptcha` возвращает `null` → понятная ошибка под полем.
- **Экран «подарок уже получен»** — `SuccessScreen`/`api.ts`: новый тип `GiftReason` (`granted` / `already_gifted` / `not_eligible` / `no_gift` / `duplicate`). При повторной заявке с того же номера показывается честный экран «Заявка принята — подарок выдаётся один раз на номер» вместо экрана без объяснений. Раньше гость думал, что заявка не прошла, и слал её снова — это и создавало повторные выдачи. Про инвентарь в этом сценарии **ничего не обещаем**: первый подарок мог быть потрачен или сгореть по сроку.

### 🔧 Исправления

- **Ошибки отправки больше не сносят лендинг** — `LandingPage`/`LeadForm`: отдельный стейт `submitError` и проп в форму — лимит запросов (`too_many_requests`) и отказ капчи показываются строкой под полем телефона, страница остаётся живой и гость может повторить. Прежний `error` подменял собой весь экран. Оба сценария логируются событием `submit_error` с кодом причины.
- **`submitLead` больше не терял `error` из ответа** — `api.ts`: поле `error` не пробрасывалось из результата webhook, из-за чего серверные коды причин не доходили до UI. Добавлены `requireCaptcha`/`captchaKey`/`retryAfter`/`giftReason` (с поддержкой обоих регистров ключей — camelCase и snake_case).
- **Дисклеймер формы упоминает SmartCaptcha** — `LeadForm`: значок капчи скрыт (`hideShield`), а условия сервиса требуют уведомить о её использовании — добавлена ссылка на условия Yandex SmartCaptcha.

### Затронутые файлы

`src/utils/captcha.ts` `[NEW]`, `src/utils/api.ts`, `src/components/LeadForm.tsx`, `src/components/SuccessScreen.tsx`, `src/pages/LandingPage.tsx`

---

## [0.3.3] — 2026-07-22 `v0.3.3`
